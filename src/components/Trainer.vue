<template>
  <div role="tablist">
    <div role="tab" class="mb-1" v-for="(mensch, index) in $root.trainer" :key="index">
      <b-button
        block
        href="#"
      ...
      <b-collapse :id="'accordion-'+index" accordion="my-accordion" role="tabpanel">
        <b-card>
          <b-card-text>
            <h3>
              {{ mensch.vorname }}
              <router-link :to="'/sportarten/'+mensch.link" class="btn btn-success float-right">
            ...
            <div>
              ...
              <p v-if="mensch.mobil" class="d-block d-sm-block d-md-none">
                ...
                <b-link
                  :href="'https://api.whatsapp.com/send?phone=+49'+forWhatsapp(mensch.mobil)"
                >WhatsApp</b-link>
              </p>
              <p v-if="mensch.mail">
                <i class="far fa-envelope" style="width:30px"></i>
                <b-link href="mailto:janice.haeusinger93@t-online.de">{{ mensch.mail }}</b-link>
              </p>
            </div>
          </b-card-text>
        </b-card>
